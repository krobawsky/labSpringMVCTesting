...
//	@Test
	...
	@Test
	public void testFindEmployeeByNAS() {

		try {
			//
			Employee emp = employeeDAO.findEmployee2("Ricardo", "Berrospi", 5000);

			Assert.assertNotNull(emp);

			Assert.assertEquals("rberrospi", emp.getLogin());
			Assert.assertEquals("awdr", emp.getPassword());
			Assert.assertEquals(7, emp.getDepartmentId());
